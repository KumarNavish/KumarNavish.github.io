"""Materialize the reviewed text delta, with immutable before/after byte checks."""
from pathlib import Path
import base64, gzip, hashlib, json, subprocess
root = Path(__file__).resolve().parents[1]
chunks = [(root / '.delivery' / f'delta-{i}.bin').read_bytes() for i in range(3)]
# Correct five recorded transport transcription defects before decoding the archive.
# The complete corrected payload must match its independently recorded SHA-256.
