...
try {
  ...
  await check("homepage gives every flagship equal access to a native mechanism", async () => {
    ...
    for (const name of ["Gain graph structure", "Experience Replay", "Rank Feasibility", "Temporal replay value", "CasePath"]) {
      ...
      assert.match(pulse, /Cause/);
      assert.match(pulse, /Measured response/);
      assert.match(pulse, /Consequence/);
    }
  ...
  await check("temporal replay lets history cross from useful to inertia", async () => {
    ...
    await sliders.nth(0).evaluate((el) => { el.value = "30"; el.dispatchEvent(new Event("change", { bubbles: true })); el.dispatchEvent(new Event("input", { bubbles: true })); });
    await sliders.nth(1).evaluate((el) => { el.value = "0.05"; el.dispatchEvent(new Event("change", { bubbles: true })); el.dispatchEvent(new Event("input", { bubbles: true })); });
    const stable = await page.locator(".mechanism-pulse").innerText();
    assert.match(stable, /earns its budget/i);
    await sliders.nth(1).evaluate((el) => { el.value = "0.95"; el.dispatchEvent(new Event("change", { bubbles: true })); el.dispatchEvent(new Event("input", { bubbles: true })); });
  ...
  await check("project pages preserve trajectory position, authorship, evidence, and boundary", async () => {
    ...
    return "came from → this work → leads toward plus contribution and limitation";
  });
