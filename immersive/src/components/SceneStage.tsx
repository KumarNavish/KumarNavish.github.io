...
export const SceneStage = forwardRef<StageHandle, Props>(function SceneStage(
  { config, description, className = "", callbacks = {}, quiet = false },
  ref,
) {
  const host = useRef<HTMLDivElement>(null),
    outer = useRef<HTMLDivElement>(null);
  const engine = useRef<ResearchRenderer | null>(null);
  const latest = useRef({ config, callbacks });
  latest.current = { config, callbacks };
  ...
  useImperativeHandle(
    ref,
    () => ({
      view: (name) => engine.current?.setView(name),
      pause: (value) => engine.current?.setPaused(value),
    }),
    [],
  );
  useEffect(() => {
    let cancelled = false,
      started = false;
    const load = async () => {
      if (started || cancelled || !host.current) return;
      started = true;
      try {
        if (new URLSearchParams(location.search).has("no3d"))
          throw new Error("3D is disabled for this accessible view.");
        const module = await import("../engine/renderer");
        if (cancelled || !host.current) return;
        engine.current = new module.ResearchRenderer(
          host.current,
          latest.current.config,
          {
            ...
            onError: (message) => {
              setError(message);
              setStatus("fallback");
              latest.current.callbacks.onError?.(message);
            },
          },
        );
        ...
        if (!cancelled) {
          ...
          setStatus("fallback");
          latest.current.callbacks.onError?.("WebGL is unavailable.");
        }
      }
    };
    ...
    >
      ...
      {status !== "ready" && (
        <div className="stage-fallback" role="status">
          ...
          {status === "fallback" && (
            <small>
              The guided text, controls, calculations, and source links remain
              available below.
            </small>
          )}
        </div>
      ...
      <div className="viewport-topline">
        <span className={status === "ready" ? "live-mark" : ""}>
          {status === "ready"
            ? "Live 3D"
            : status === "loading"
              ? "Preparing scene"
        ...
        {config.kind === "world" && (
          <span>Persistent world · r{config.world?.revision ?? 0}</span>
        )}
      </div>
