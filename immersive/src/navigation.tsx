...
const ALIASES: Record<string, string> = {
  ...
  "/labs/spatial-intelligence": "/frontier/spatial-intelligence",
  "/research/graph-laplacians": "/work/normalized-gain-laplacians",
  "/work/gain-graphs": "/work/normalized-gain-laplacians",
  "/research/gbr": "/work/experience-replay-optimization",
  "/research/experience-replay-optimization":
...
export function metadata(path: string) {
  const e = exhibitByPath(cleanPath(path));
  const title = e
    ? `${e.name} — Navish Kumar`
    : path === "/trajectory"
      ? "Research trajectory — Navish Kumar"
      : path === "/work"
        ? "All work — Navish Kumar"
        : path === "/research"
          ? "Research record — Navish Kumar"
          : path === "/systems"
            ? "Systems — Navish Kumar"
            : path === "/about"
              ? "About — Navish Kumar"
              : path === "/frontier"
                ? "Frontier — Navish Kumar"
                : "Navish Kumar — Machine-learning researcher & systems builder";
  return {
    title,
    description: e
      ? e.question
      : "Explore research in optimization, continual learning, evidence-grounded systems, and persistent spatial interfaces through live 3D explanations.",
  };
}
...
export function NavigationProvider({
  ...
  useEffect(() => {
    const handler = (e: PopStateEvent) => {
      scroll.current = e.state?.scrollY ?? 0;
  ...
  useEffect(() => {
    ...
    canonical?.setAttribute(
      "href",
      `https://kumarnavish.github.io${path === "/" ? "/" : path + "/"}`,
    );
