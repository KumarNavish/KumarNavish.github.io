...
export function timeScene(view,overlay){
 ...
 return {render(f){const s=f.science,r=f.reveal,t=f.exploring?s.month:f.visual.time;
  windows.forEach((w,i)=>{const present=i===s.month,age=s.month-i,keep=age>0&&age<=s.window;w.position.set((i-t)*.73+1.4,.10+meanAt(i)*.37,-.35);alpha(w,i<=s.month?present?1:keep?.78:.22:0);w.children[0].material.color.set(present?'#5c937a':keep?(s.benefit<0?'#bb7b58':'#ad9c72'):'#c5bba9');});
  ...
  const a=[{text:`Present · period ${s.month}`,pos:[1.4,2.0+meanAt(s.month)*.37,-.35],tone:'green'}];if(r.risk>.7&&Math.abs(s.bias)>.02)a.push({text:'The archive pulls toward an outdated mean',pos:[1.4,.55+(s.current+s.bias)*.7,-1.5],tone:'red'});labels(a);
