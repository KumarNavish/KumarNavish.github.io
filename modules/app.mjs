import {byId} from './content.mjs?v=scroll-4';
import {home,detail,notFound} from './render.mjs?v=scroll-4';
...
async function route(initial=false){
 ...
 if(hash.startsWith('work/')){
  ...
  try{const {mountScrollNarrative}=await import('./narrative/director.mjs?v=scroll-4');if(seq!==sequence)return;const n=mountScrollNarrative(w,experience.firstElementChild,{at,chapter});dispose=()=>n.dispose();}catch(error){experience.innerHTML='<p class="world-load-error">The interactive explanation could not load. The original sources and contribution remain available below.</p>';console.error(error);}
 }else{
  ...
  try{const{mountJourney}=await import('./narrative/journey.mjs?v=scroll-4');if(seq!==sequence)return;dispose=mountJourney(main);}catch(error){console.error(error);}
