...
with sync_playwright() as pw:
 ...
 try:
  for width,height in [(1440,1000),(390,844),(320,568)]:
   ...
   for mode in range(5):
    page.locator('[data-select="eigenmode"]').select_option(str(mode));page.wait_for_timeout(100)
    reading=page.evaluate('''async()=>{
     const T=await import('./vendor/three.module.min.js');const {gainNodePositions}=await import('./modules/narrative/graph-scenes.mjs');const {stories}=await import('./modules/narrative/chapters.mjs');
     const host=document.querySelector('.n-canvas-host'),r=host.getBoundingClientRect(),canvas=host.querySelector('canvas'),camera=new T.PerspectiveCamera(40,r.width/r.height,.08,180);camera.position.fromArray(canvas.dataset.camera.split(',').map(Number));camera.lookAt(...stories.gain.targets.at(-1));camera.updateMatrixWorld();
     ...
     const obstacles=[...document.querySelector('.n-scene-overlay').children,host.querySelector('.camera-tools')].filter(e=>e&&visible(e)).map(e=>{const a=e.getBoundingClientRect();return{left:a.left-r.left,right:a.right-r.left,top:a.top-r.top,bottom:a.bottom-r.top,name:e.className};});
     const projected=[];
     ...
     return{projected,obstacles,mode:mode.index,residual:mode.residual,canvas:[r.width,r.height]};
    }''')
   ...
   page.locator('[data-select="eigenmode"]').select_option('-1');page.wait_for_timeout(100)
