...
import argparse, json, hashlib, time, urllib.request
...
with sync_playwright() as p:
 ...
 def shot(work):
  page.evaluate("document.activeElement?.blur();scrollTo({top:0,behavior:'instant'})");page.screenshot(path=str(out/(work+'-page.png')),full_page=True);page.locator('.world-exhibit').screenshot(path=str(out/(work+'.png')))
 for work in ids:
  go(work)
  if work=='gain-graphs':
   ...
   click('next');click('memory1');before=state();check('duplicate memory adds no independent direction',before['directions']==1 and before['missing']>0);shot(work);click('next');check('complete memory basis removes residual',state()['missing']<1e-8 and state()['directions']==3)
