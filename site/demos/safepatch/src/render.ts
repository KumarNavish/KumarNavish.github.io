...
const RAW_COLOR = '#ce4f63'
const SAFE_COLOR = '#2a69cf'
const ZONE_COLOR = '#118362'
const CORRECTION_COLOR = '#dc8b39'
const GRID_STROKE = 'rgba(92, 121, 154, 0.13)'
...
export class SceneRenderer {
  ...
  render(input: SceneRenderInput): void {
    ...
    if (stacked) {
      const topHeight = Math.max(220, Math.round((height - margin * 2 - gap) * 0.5))
      geometryPanel = {
        x: margin,
        y: margin,
    ...
    const pulse = 0.5 + 0.5 * Math.sin(input.clockMs * 0.0023)
    ...
    this.drawPanelShell(geometryPanel, 'PATCH SPACE', 'raw vector vs corrected vector')
    this.drawPanelShell(queuePanel, 'QUEUE REPLAY', 'before and after under identical load')
  ...
  private paintBackdrop(width: number, height: number): void {
    ...
    linear.addColorStop(0, '#fbfdff')
    linear.addColorStop(0.58, '#eff4fd')
    linear.addColorStop(1, '#e8eef8')
    ctx.fillStyle = linear
    ctx.fillRect(0, 0, width, height)

    const leftGlow = ctx.createRadialGradient(width * 0.08, height * 0.1, 20, width * 0.08, height * 0.1, width * 0.5)
    leftGlow.addColorStop(0, 'rgba(138, 171, 219, 0.2)')
    leftGlow.addColorStop(1, 'rgba(138, 171, 219, 0)')
  ...
  private drawPanelShell(panel: Rect, title: string, subtitle: string): void {
    ...
    this.drawRoundedRect(panel.x, panel.y, panel.width, panel.height, 14)
    ctx.fillStyle = 'rgba(255, 255, 255, 0.76)'
    ctx.fill()
    ctx.strokeStyle = 'rgba(132, 158, 190, 0.34)'
    ctx.lineWidth = 1
    ctx.stroke()

    const accent = ctx.createLinearGradient(panel.x, panel.y, panel.x + panel.width * 0.7, panel.y)
    accent.addColorStop(0, 'rgba(53, 106, 189, 0.24)')
    accent.addColorStop(1, 'rgba(53, 106, 189, 0)')
    ctx.fillStyle = accent
    ctx.fillRect(panel.x + 12, panel.y + 10, Math.max(80, panel.width * 0.36), 2)

    ctx.font = '700 10px "IBM Plex Mono", monospace'
    ctx.fillStyle = '#3d6088'
    ctx.fillText(title, panel.x + 12, panel.y + 24)

    ctx.font = '600 10px "Manrope", sans-serif'
    ctx.fillStyle = 'rgba(82, 106, 136, 0.9)'
  ...
  private drawGeometryPanel(
    ...
    const zone = intersectHalfspaces(activeHalfspaces, mapper.worldRadius)
    ...
    if (!zone.isEmpty) {
      ...
      fill.addColorStop(0, withAlpha(ZONE_COLOR, 0.1 + pulse * 0.06))
      fill.addColorStop(1, withAlpha(ZONE_COLOR, 0.03))
      ctx.fillStyle = fill
      ctx.fill()
    ...
    this.drawConstraintBoundaries(chart, mapper, activeHalfspaces)
    ...
    this.drawArrow(origin, rawEnd, RAW_COLOR, 2, true)
    this.drawArrow(origin, safeEnd, SAFE_COLOR, 2.8, false)
    this.drawDirectionHint(mapper, gradient)

    this.drawCorrectionCurve(rawEnd, safeEnd)
    this.drawProjectionPulse(rawEnd, safeEnd, blend, pulse)

    this.drawVectorTag(rawEnd, 'raw')
    this.drawVectorTag(safeEnd, 'safe')

    ctx.beginPath()
    ctx.arc(origin.x, origin.y, 3.6, 0, Math.PI * 2)
    ctx.fillStyle = '#33597f'
    ctx.fill()
  }
  ...
  private drawConstraintBoundaries(rect: Rect, mapper: Mapper, halfspaces: Halfspace[]): void {
    ...
    halfspaces.forEach((halfspace, index) => {
      ...
      ctx.strokeStyle = withAlpha(index % 2 === 0 ? '#74addd' : '#5cab89', 0.44)
      ctx.lineWidth = 0.9
      ctx.stroke()
    })
    ...
    ctx.strokeStyle = withAlpha('#648fb7', 0.36)
    ctx.lineWidth = 0.9
    ctx.stroke()
  ...
  private drawDirectionHint(mapper: Mapper, gradient: Vec2): void {
    ...
    this.drawArrow(origin, target, withAlpha('#c07c35', 0.58), 1.2, true)
  }

  private drawCorrectionCurve(rawEnd: Vec2, safeEnd: Vec2): void {
    const ctx = this.ctx
    ...
    ctx.strokeStyle = withAlpha(CORRECTION_COLOR, 0.75)
    ctx.lineWidth = 1.2
    ctx.stroke()
    ctx.restore()
  }

  private drawProjectionPulse(rawEnd: Vec2, safeEnd: Vec2, blend: number, pulse: number): void {
    const ctx = this.ctx
    const midX = (rawEnd.x + safeEnd.x) / 2
    const midY = (rawEnd.y + safeEnd.y) / 2 - 16
    ...
    ctx.beginPath()
    ctx.arc(x, y, 3.4 + pulse * 1.8, 0, Math.PI * 2)
    ctx.fillStyle = withAlpha(CORRECTION_COLOR, 0.92)
    ctx.fill()

    ctx.beginPath()
    ctx.arc(x, y, 8.4 + pulse * 2.2, 0, Math.PI * 2)
    ctx.strokeStyle = withAlpha(CORRECTION_COLOR, 0.26 + pulse * 0.15)
    ctx.lineWidth = 1.05
    ctx.stroke()
  }

  private drawVectorTag(point: Vec2, label: string): void {
    const ctx = this.ctx
    ...
    ctx.fill()
    ctx.strokeStyle = 'rgba(102, 128, 157, 0.42)'
    ctx.lineWidth = 1
    ctx.stroke()

    ctx.fillStyle = '#3d5f86'
  ...
  private drawQueuePanel(
    ...
    this.drawClipped(chart, reveal, () => {
      ...
      this.drawSmoothSeries(rawSeries, mapX, mapY, RAW_COLOR, 2.1)
    ...
    const rawTail = rawSeries[rawSeries.length - 1]
    const safeTail = safeSeries[safeSeries.length - 1]
    const saved = Math.max(0, Math.round(rawTail - safeTail))
    this.drawPill(chart.x + 8, chart.y + 8, `HORIZON SAVED ${saved}`)
  }
  ...
  private drawThresholdZone(chart: Rect, thresholdY: number): void {
    ...
    ctx.fillStyle = '#854354'
    ctx.font = '600 10px "IBM Plex Mono", monospace'
    ctx.fillText('risk threshold', chart.x + chart.width - 88, thresholdY - 6)
  }
  ...
  private drawSweep(chart: Rect, reveal: number, pulse: number): void {
    ...
    const gradient = ctx.createLinearGradient(x - 20, chart.y, x + 20, chart.y)
    gradient.addColorStop(0, 'rgba(46, 146, 244, 0)')
    gradient.addColorStop(0.5, `rgba(46, 146, 244, ${0.07 + pulse * 0.06})`)
    gradient.addColorStop(1, 'rgba(46, 146, 244, 0)')
    ctx.fillStyle = gradient
    ctx.fillRect(x - 20, chart.y, 40, chart.height)
  }
  ...
  private drawPill(x: number, y: number, text: string): void {
    ...
    ctx.fill()
    ctx.strokeStyle = 'rgba(76, 110, 148, 0.42)'
    ctx.lineWidth = 1
    ctx.stroke()

    ctx.fillStyle = '#315983'
