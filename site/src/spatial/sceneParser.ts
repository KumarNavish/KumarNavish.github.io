...
function relationFor(kind: SceneObjectKind, text: string): string | undefined {
  ...
  const beside = new RegExp(
    `${label}[^.]{0,36}(?:beside|next to|near) (?:a |the )?([a-z -]+)`,
    'i',
  ).exec(text)
  if (beside?.[1]) return `beside ${beside[1].trim().split(/,| and /)[0]}`
  const behind = new RegExp(
    `${label}[^.]{0,36}(?:behind|in front of|above) (?:a |the )?([a-z -]+)`,
    'i',
  ).exec(text)
...
export function parseSceneCommand(raw: string): ParsedIntent {
  ...
  const additions = removeMode
    ? []
    : unique(mentioned).map((kind) => ({ kind, relation: relationFor(kind, text) }))
  const targetKind = OBJECT_TERMS.find(([, pattern]) => pattern.test(text))?.[0]
  let action: ParsedIntent['action']
  if (/\binspect\b/i.test(text))
    action = { verb: 'inspect', targetKind: /sample|specimen/i.test(text) ? 'sample' : targetKind }
  ...
  return {
    ...
    operation: removeMode
      ? 'remove'
      : /\b(?:create|make|build|generate)\b/i.test(text)
        ? 'create'
        : 'edit',
    environment: detectEnvironment(text),
    timeOfDay: detectTime(text),
    mood: /\b(?:quiet|calm|peaceful)\b/i.test(text)
      ? 'quiet'
      : /\b(?:busy|active|lively)\b/i.test(text)
        ? 'active'
        : undefined,
    additions,
    removals,
    action,
    confidence: Math.min(0.98, 0.48 + recognized * 0.09),
...
export function applySceneIntent(world: WorldState, intent: ParsedIntent): WorldState {
  ...
  for (const addition of intent.additions) {
    if (addition.kind === 'laboratory' && objects.some((object) => object.kind === 'laboratory'))
      continue
    const [x, y] = targetPosition(addition.kind, objects)
    objects = [
      ...objects,
      {
        id: nextId(addition.kind, objects),
        kind: addition.kind,
        label: LABELS[addition.kind],
        x,
        y,
  ...
  if (intent.action) {
    const target = intent.action.targetKind
      ? objects.find((object) => object.kind === intent.action?.targetKind)
      : undefined
...
export function parseAndApply(
  world: WorldState,
  command: string,
): { intent: ParsedIntent; world: WorldState } {
...
export function moveWorldObject(world: WorldState, id: string, x: number, y: number): WorldState {
  return {
    ...world,
    objects: world.objects.map((object) => (object.id === id ? { ...object, x, y } : object)),
    revision: world.revision + 1,
  }
}
