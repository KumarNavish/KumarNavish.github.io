...
export function parseLatestRunApi(payload: unknown): LatestRunApi {
  ...
  const triggerRaw =
    run.trigger && typeof run.trigger === 'object' && !Array.isArray(run.trigger)
      ? (run.trigger as JsonObject)
      : null
  ...
  return {
    run: {
      ...
      trigger: triggerRaw
        ? {
            event_name:
              typeof triggerRaw.event_name === 'string' ? triggerRaw.event_name : undefined,
            is_scheduled:
              typeof triggerRaw.is_scheduled === 'boolean'
                ? triggerRaw.is_scheduled
                : undefined,
            run_attempt:
              typeof triggerRaw.run_attempt === 'string' ? triggerRaw.run_attempt : undefined,
            schedule: typeof triggerRaw.schedule === 'string' ? triggerRaw.schedule : undefined,
            workflow: typeof triggerRaw.workflow === 'string' ? triggerRaw.workflow : undefined,
          }
        : undefined,
    },
    summary: {
    ...
    tasks: tasksRaw.map((taskRaw, index) => {
      ...
      return {
        ...
        duration_seconds: asNumber(
          ...
          `latest run.tasks[${index}].duration_seconds`,
        ),
        logs: logsRaw
...
export function parseProvenanceApi(payload: unknown): ProvenanceApi {
  ...
  const triggerRaw =
    root.trigger && typeof root.trigger === 'object' && !Array.isArray(root.trigger)
      ? (root.trigger as JsonObject)
      : null
  return {
    ...
    artifacts: Object.fromEntries(
      Object.entries(asObject(root.artifacts ?? {}, 'provenance.artifacts')).map(
    ...
    trigger: triggerRaw
      ? {
          event_name:
            typeof triggerRaw.event_name === 'string' ? triggerRaw.event_name : undefined,
          is_scheduled:
            typeof triggerRaw.is_scheduled === 'boolean' ? triggerRaw.is_scheduled : undefined,
          run_attempt:
            typeof triggerRaw.run_attempt === 'string' ? triggerRaw.run_attempt : undefined,
          schedule: typeof triggerRaw.schedule === 'string' ? triggerRaw.schedule : undefined,
          workflow: typeof triggerRaw.workflow === 'string' ? triggerRaw.workflow : undefined,
...
export function parseDagApi(payload: unknown): DagApi {
  ...
  if (!Array.isArray(tasksRaw) || !Array.isArray(edgesRaw)) {
    throw new Error('invalid dag payload: tasks and edges must be arrays')
  }
  return {
